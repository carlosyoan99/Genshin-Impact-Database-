import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { Item, Language } from '../../types';
import { getItemsByCategory } from '../../services/genshinService';
import { motion, AnimatePresence } from 'framer-motion';
import { ArrowLeft, Utensils, Info, Loader2, Sparkles, ChefHat } from 'lucide-react';
import { UI_TRANSLATIONS } from '../../constants';

interface FoodDetailProps {
  language: Language;
}

const FoodDetail: React.FC<FoodDetailProps> = ({ language }) => {
  const { foodId } = useParams<{ foodId: string }>();
  const navigate = useNavigate();
  const t = UI_TRANSLATIONS[language];
  const [food, setFood] = useState<Item | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchFood = async () => {
      setLoading(true);
      try {
        const foods = await getItemsByCategory('food', language);
        const found = foods.find(f => f.id === foodId);
        setFood(found || null);
      } catch (error) {
        console.error('Error fetching food detail:', error);
      } finally {
        setLoading(false);
      }
    };
    fetchFood();
  }, [foodId, language]);

  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 min-h-screen bg-[#0d0e12] text-gray-500">
        <Loader2 size={64} className="text-amber-500 animate-spin" />
        <p className="text-xl font-medium">Cooking recipe details...</p>
      </div>
    );
  }

  if (!food) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 min-h-screen bg-[#0d0e12] text-gray-500">
        <Info size={64} className="text-red-500 opacity-50" />
        <p className="text-xl font-medium">Food not found in Teyvat.</p>
        <button onClick={() => navigate('/foods')} className="bg-amber-500 text-white px-6 py-3 rounded-2xl font-bold hover:bg-amber-600 transition-colors">
          Back to Foods
        </button>
      </div>
    );
  }

  return (
    <div className="flex-1 min-h-screen bg-[#0d0e12]">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-12 space-y-12">
        <motion.button 
          initial={{ x: -20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          onClick={() => navigate('/foods')}
          className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors group"
        >
          <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
          <span className="font-bold uppercase tracking-widest text-xs">Back to Foods</span>
        </motion.button>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          {/* Image Section */}
          <motion.div 
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            className="relative aspect-square bg-gray-800/20 rounded-[40px] border border-gray-800/50 flex items-center justify-center overflow-hidden group"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
            {food.icon && (
              <img 
                src={food.icon} 
                alt={food.name} 
                className="w-3/4 h-3/4 object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] group-hover:scale-110 transition-transform duration-500"
                referrerPolicy="no-referrer"
              />
            )}
          </motion.div>

          {/* Info Section */}
          <div className="space-y-10">
            <div className="space-y-4">
              <motion.div 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                className="flex items-center gap-3"
              >
                <span className="text-gray-400 font-bold uppercase tracking-widest text-xs">Food Type: {food.foodType || 'Unknown'}</span>
              </motion.div>

              <motion.h1 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.1 }}
                className="text-5xl sm:text-7xl font-black text-white tracking-tight"
              >
                {food.name}
              </motion.h1>

              <motion.p 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ delay: 0.2 }}
                className="text-gray-400 text-lg leading-relaxed italic"
              >
                {food.description}
              </motion.p>
            </div>

            {food.effect && (
              <div className="bg-amber-500/5 p-8 rounded-[32px] border border-amber-500/20 space-y-4">
                <h3 className="text-amber-500 font-black uppercase tracking-widest text-xs flex items-center gap-2">
                  <Sparkles size={16} />
                  Food Effect
                </h3>
                <p className="text-gray-200 leading-relaxed">
                  {food.effect}
                </p>
              </div>
            )}

            {food.recipe && (
              <div className="space-y-4">
                <h3 className="text-white font-black uppercase tracking-widest text-xs flex items-center gap-2">
                  <ChefHat size={16} className="text-amber-500" />
                  Recipe / Ingredients
                </h3>
                <div className="bg-gray-800/30 p-6 rounded-3xl border border-gray-800/50">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {Array.isArray(food.recipe) ? food.recipe.map((ing, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 bg-gray-900/50 rounded-xl border border-gray-700/50">
                        <span className="text-gray-200 font-medium">{ing.name}</span>
                        <span className="text-amber-500 font-black">x{ing.count}</span>
                      </div>
                    )) : <p className="text-gray-300 leading-relaxed">{food.recipe}</p>}
                  </div>
                </div>
              </div>
            )}
          </div>
        </div>
      </div>
    </div>
  );
};

export default FoodDetail;
