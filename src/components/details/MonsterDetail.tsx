...
import { ArrowLeft, Ghost, Info, Loader2, MapPin, Zap } from 'lucide-react';
...
const MonsterDetail: React.FC<MonsterDetailProps> = ({ language }) => {
  const { monsterId } = useParams<{ monsterId: string }>();
  const navigate = useNavigate();
  const t = UI_TRANSLATIONS[language];
  const [monster, setMonster] = useState<Item | null>(null);
  ...
  useEffect(() => {
    const fetchMonster = async () => {
      setLoading(true);
      try {
        const monsters = await getItemsByCategory('enemies', language);
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 min-h-screen bg-[#0d0e12] text-gray-500">
        ...
        <p className="text-xl font-medium">Tracking monster details...</p>
      </div>
    );
  }
  ...
  if (!monster) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-6 min-h-screen bg-[#0d0e12] text-gray-500">
        ...
        <p className="text-xl font-medium">Monster not found in Teyvat.</p>
        <button onClick={() => navigate('/monsters')} className="bg-amber-500 text-white px-6 py-3 rounded-2xl font-bold hover:bg-amber-600 transition-colors">
          Back to Monsters
        </button>
      </div>
    );
    ...
    <div className="flex-1 min-h-screen bg-[#0d0e12]">
      <div className="max-w-7xl mx-auto px-6 sm:px-10 py-12 space-y-12">
        ...
        >
          ...
          <span className="font-bold uppercase tracking-widest text-xs">Back to Monsters</span>
        </motion.button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-10">
            <div className="space-y-4">
              ...
              >
                <span className="text-gray-400 font-bold uppercase tracking-widest text-xs">Enemy Type: {monster.enemyType || 'Unknown'}</span>
              </motion.div>
              ...
              {monster.area && (
                <div className="bg-gray-800/30 p-6 rounded-3xl border border-gray-800/50 space-y-2">
                  <div className="flex items-center gap-2 text-amber-500">
                    <MapPin size={18} />
                    <span className="text-xs font-bold uppercase tracking-widest">Common Area</span>
                  </div>
                  <p className="text-xl font-bold text-white">{monster.area}</p>
                </div>
              )}
              {monster.element && (
                <div className="bg-gray-800/30 p-6 rounded-3xl border border-gray-800/50 space-y-2">
                  <div className="flex items-center gap-2 text-amber-500">
                    <Zap size={18} />
                    <span className="text-xs font-bold uppercase tracking-widest">Element</span>
                  </div>
                  <p className="text-xl font-bold text-white">{monster.element}</p>
                </div>
              )}
            </div>
          </div>
