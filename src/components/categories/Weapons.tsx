...
import { Sparkles, Filter, LayoutGrid, List as ListIcon, Loader2, Search, Swords } from 'lucide-react';
...
const Weapons: React.FC<WeaponsProps> = ({ language }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await getItemsByCategory('weapons', language);
        setItems(data);
      } catch (error) {
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      const matchesType = activeWeaponType === 'all' || item.weaponType === activeWeaponType;
      const matchesRarity = activeRarity === 'all' || item.rarity?.toString() === activeRarity;
      return matchesSearch && matchesType && matchesRarity;
    });
  }, [items, searchQuery, activeWeaponType, activeRarity]);
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-4xl sm:text-6xl font-black text-white tracking-tight flex items-center gap-4">
                {t.weapons}
                <Swords className="text-amber-500 w-8 h-8" />
              </h1>
              <p className="text-gray-500 font-medium text-lg">
                Equip your characters with powerful armaments from across Teyvat.
              </p>
            </div>
          ...
          {loading ? (
            <div className="h-[50vh] flex flex-col items-center justify-center gap-4 text-gray-500">
              ...
              <p className="text-lg font-medium">Forging weapons...</p>
            </div>
              ...
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item) => (
                  <ItemCard
                    ...
                    item={item}
                    language={language}
                    onClick={() => navigate(`/weapons/${item.id}`)}
                  />
                ))}
              </AnimatePresence>
            </motion.div>
            ...
            <div className="h-[40vh] flex flex-col items-center justify-center gap-4 text-gray-500 bg-gray-800/5 rounded-3xl border-2 border-dashed border-gray-800/50">
              ...
              <p className="text-xl font-medium">No weapons found matching your filters.</p>
              <button 
                onClick={() => { setActiveWeaponType('all'); setActiveRarity('all'); setSearchQuery(''); }}
