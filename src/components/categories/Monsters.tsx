...
import { Sparkles, Filter, LayoutGrid, List as ListIcon, Loader2, Search, Ghost } from 'lucide-react';
...
const Monsters: React.FC<MonstersProps> = ({ language }) => {
  ...
  const [activeEnemyType, setActiveEnemyType] = useState<string>('all');

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await getItemsByCategory('enemies', language);
        setItems(data);
      } catch (error) {
  ...
  const enemyTypes = useMemo(() => ['all', ...Array.from(new Set(items.map(i => i.enemyType).filter(Boolean)))], [items]);

  const filteredItems = useMemo(() => {
    return items.filter(item => {
      const matchesSearch = item.name.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesType = activeEnemyType === 'all' || item.enemyType === activeEnemyType;
      return matchesSearch && matchesType;
    });
  }, [items, searchQuery, activeEnemyType]);
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-4xl sm:text-6xl font-black text-white tracking-tight flex items-center gap-4">
                {t.enemies || 'Monsters'}
                <Ghost className="text-amber-500 w-8 h-8" />
              </h1>
              <p className="text-gray-500 font-medium text-lg">
                Study your foes to conquer the challenges of Teyvat.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 bg-gray-800/20 p-3 rounded-2xl border border-gray-800/50 backdrop-blur-md">
              ...
              <select 
                value={activeEnemyType} 
                onChange={(e) => setActiveEnemyType(e.target.value)}
              ...
              >
                ...
                {enemyTypes.filter(e => e !== 'all').map(e => <option key={e as string} value={e as string}>{e as string}</option>)}
              </select>
            </div>
          </div>
          ...
          {loading ? (
            <div className="h-[50vh] flex flex-col items-center justify-center gap-4 text-gray-500">
              ...
              <p className="text-lg font-medium">Tracking monsters...</p>
            </div>
              ...
              <AnimatePresence mode="popLayout">
                {filteredItems.map((item) => (
                  <ItemCard
                    ...
                    item={item}
                    language={language}
                    onClick={() => navigate(`/monsters/${item.id}`)}
                  />
                ))}
              </AnimatePresence>
            </motion.div>
            ...
            <div className="h-[40vh] flex flex-col items-center justify-center gap-4 text-gray-500 bg-gray-800/5 rounded-3xl border-2 border-dashed border-gray-800/50">
              ...
              <p className="text-xl font-medium">No monsters found matching your filters.</p>
              <button 
                onClick={() => { setActiveEnemyType('all'); setSearchQuery(''); }}
