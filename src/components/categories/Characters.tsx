...
import { Sparkles, Filter, LayoutGrid, List as ListIcon, Loader2, Search, Users } from 'lucide-react';
...
const Characters: React.FC<CharactersProps> = ({ language }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await getItemsByCategory('characters', language);
        setItems(data);
      } catch (error) {
  ...
  const elements = useMemo(() => ['all', ...Array.from(new Set(items.map(i => i.element).filter(Boolean)))], [items]);
  const weaponTypes = useMemo(() => ['all', ...Array.from(new Set(items.map(i => i.weaponType).filter(Boolean)))], [items]);
  const rarities = useMemo(() => ['all', ...Array.from(new Set(items.map(i => i.rarity?.toString()).filter(Boolean)))], [items]);

  const filteredItems = useMemo(() => {
    return items.filter(item => {
      const matchesSearch = item.name.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesElement = activeElement === 'all' || item.element === activeElement;
      const matchesWeapon = activeWeapon === 'all' || item.weaponType === activeWeapon;
  ...
  }, [items, searchQuery, activeElement, activeWeapon, activeRarity]);
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-gray-500 font-medium text-lg">
                Discover the heroes of Teyvat and their unique abilities.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 bg-gray-800/20 p-3 rounded-2xl border border-gray-800/50 backdrop-blur-md">
              ...
              >
                ...
                {elements.filter(e => e !== 'all').map(e => <option key={e as string} value={e as string}>{e as string}</option>)}
              </select>

              <select 
                value={activeWeapon} 
                onChange={(e) => setActiveWeapon(e.target.value)}
                className="bg-gray-900/50 border border-gray-700 text-gray-300 text-xs font-bold uppercase tracking-wider px-3 py-2 rounded-xl outline-none focus:border-amber-500 transition-colors"
              >
                <option value="all">Weapons</option>
                {weaponTypes.filter(w => w !== 'all').map(w => <option key={w as string} value={w as string}>{w as string}</option>)}
              </select>
          ...
          {loading ? (
            <div className="h-[50vh] flex flex-col items-center justify-center gap-4 text-gray-500">
              ...
              <p className="text-lg font-medium">Summoning characters...</p>
            </div>
            ...
            <div className="h-[40vh] flex flex-col items-center justify-center gap-4 text-gray-500 bg-gray-800/5 rounded-3xl border-2 border-dashed border-gray-800/50">
              ...
              <p className="text-xl font-medium">No characters found matching your filters.</p>
              <button 
                onClick={() => { setActiveElement('all'); setActiveWeapon('all'); setActiveRarity('all'); setSearchQuery(''); }}
