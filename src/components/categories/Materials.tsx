...
import { Sparkles, Filter, LayoutGrid, List as ListIcon, Loader2, Search, Briefcase } from 'lucide-react';
...
const Materials: React.FC<MaterialsProps> = ({ language }) => {
  ...
  const [activeMaterialType, setActiveMaterialType] = useState<string>('all');

  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await getItemsByCategory('materials', language);
        setItems(data);
      } catch (error) {
  ...
  const materialTypes = useMemo(() => ['all', ...Array.from(new Set(items.map(i => i.materialType).filter(Boolean)))], [items]);

  const filteredItems = useMemo(() => {
    return items.filter(item => {
      const matchesSearch = item.name.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesType = activeMaterialType === 'all' || item.materialType === activeMaterialType;
      return matchesSearch && matchesType;
    });
  }, [items, searchQuery, activeMaterialType]);
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div className="space-y-2">
              <h1 className="text-4xl sm:text-6xl font-black text-white tracking-tight flex items-center gap-4">
                ...
                <Briefcase className="text-amber-500 w-8 h-8" />
              </h1>
              <p className="text-gray-500 font-medium text-lg">
                Collect essential items for ascension and crafting.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 bg-gray-800/20 p-3 rounded-2xl border border-gray-800/50 backdrop-blur-md">
              ...
              >
                ...
                {materialTypes.filter(e => e !== 'all').map(e => <option key={e as string} value={e as string}>{e as string}</option>)}
              </select>
            </div>
          </div>
          ...
          {loading ? (
            <div className="h-[50vh] flex flex-col items-center justify-center gap-4 text-gray-500">
              ...
              <p className="text-lg font-medium">Gathering materials...</p>
            </div>
            ...
            <div className="h-[40vh] flex flex-col items-center justify-center gap-4 text-gray-500 bg-gray-800/5 rounded-3xl border-2 border-dashed border-gray-800/50">
              ...
              <p className="text-xl font-medium">No materials found matching your filters.</p>
              <button 
                onClick={() => { setActiveMaterialType('all'); setSearchQuery(''); }}
